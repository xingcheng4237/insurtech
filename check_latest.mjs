...
const latestLog = await db.select().from(collectionLogs).orderBy(desc(collectionLogs.collectionTime)).limit(1);
...
// Get latest report
const latestReport = await db.select().from(reports).orderBy(desc(reports.collectionTime)).limit(1);
