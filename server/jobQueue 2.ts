...
class JobQueue {
  ...
  private async runNewsCollection(jobId: string): Promise<any> {
    ...
    const { generateDailyNewsEmail } = await import(
    ...
    console.log(
      `[NewsCollection] Collected ${result.articles.length} articles`
    );
    ...
    console.log(
      `[NewsCollection] AHA-relevant articles: ${ahaArticles.length}/${flaggedArticles.length}`
    );
    ...
    if (emailService.isReady()) {
      try {
        ...
        if (isTestMode) {
          ...
          console.log(
            "[NewsCollection] 🧪 TEST MODE - Sending to test email only"
          );
          ...
          console.log(
            "[NewsCollection] 📧 PRODUCTION MODE - Sending to all subscribers"
          );
          const { getActiveSubscribers } = await import(
            "./subscriptionService"
          );
          ...
          if (subscribers.length === 0) {
            ...
            console.warn(
              "   Add subscribers at: /api/trpc/subscription.subscribe"
            );
          } else {
            console.log(
              `[NewsCollection] Sending to ${subscribers.length} subscriber(s)...`
            );
            ...
            console.log(
              `[NewsCollection] 📊 Emails sent: ${emailsSentCount}/${subscribers.length}`
            );
          }
        }
      } catch (error) {
      ...
      console.warn(
        "[NewsCollection] Email not configured - skipping email delivery"
      );
  ...
  private async sendSlackNotification(
    ...
    if (!webhookUrl) {
      console.log(
        "[Slack] No SLACK_WEBHOOK_URL configured — skipping Slack notification"
      );
      return;
    }
    ...
    try {
      ...
      const top5 = (ahaArticles.length >= 3 ? ahaArticles : articles).slice(
        0,
        5
      );

      const articleBlocks = top5
        .map(a => `• <${a.url}|${a.title}> — _${a.source}_`)
        .join("\n");

      const modeTag = isTestMode ? " [TEST]" : "";
      const ahaTag =
        ahaArticles.length > 0 ? ` • ${ahaArticles.length} AHA-relevant` : "";
        ...
        blocks: [
          {
            ...
            text: {
              type: "plain_text",
              text: `📰 Insurtech Weekly Digest${modeTag} — ${today}`,
              emoji: true,
            },
          },
          {
            ...
            text: {
              type: "mrkdwn",
              text: `*${articles.length} articles collected${ahaTag}*`,
            },
          },
          {
            type: "section",
